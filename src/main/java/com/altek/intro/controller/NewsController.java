...
public class NewsController {
    ...
    public ResponseEntity<BaseResponse> createNews(@RequestBody NewsRequestDto request) {
        ...
        try {
            log.info("#NewsController.createNews: START -- ");
            response = newsService.createNews(request);
            return new ResponseEntity<>(response, HttpStatus.CREATED);
        } catch (ParameterIllegalException ex) {
            ex.printStackTrace();
            log.error("#NewsController.createNews ParameterIllegalException : " + ex.getMessage(), ex);
            ...
            ex.printStackTrace();
            log.error("#NewsController.createNews AuthorizedException : " + ex.getMessage(), ex);
            response.setHttp_code(HttpStatus.UNAUTHORIZED.toString());
            response.setMessage("common.error.unauthorized");
            return new ResponseEntity<>(response ,HttpStatus.UNAUTHORIZED);
        } catch (SystemErrorException ex) {
            ex.printStackTrace();
            log.error("#NewsController.createNews SystemErrorException : " + ex.getMessage(), ex);
